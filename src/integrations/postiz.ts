...
async function uploadGraphic(post: WorkspacePost): Promise<PostizUpload> {
  ...
  const graphic = post.frozenGraphicPath
    ...
    : new Uint8Array(await (await renderTemplateGraphic({ topicHeading: post.topicHeading, articleTitle: post.articleTitle, imageUrl: post.generatedImageUrl || post.featuredImageUrl, graphicGuidance: post.graphicGuidance, sourceImageHasText: post.sourceImageHasText, adjustments: post.graphicAdjustments })).arrayBuffer());
