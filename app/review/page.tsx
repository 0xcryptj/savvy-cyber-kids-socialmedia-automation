...
export default function ReviewPage() {
  ...
  // Bumped after a layout save so the preview and download refetch, since the
  // graphic path itself never changes.
  ...
  const graphicSrc = `${post.graphicPath}${graphicVersion ? `?v=${graphicVersion}` : ""}`;
  ...
  return <>
    ...
    <div className="review-layout">
      <article className="card preview-card">{sourceWarning ? <div className="warning-panel">{sourceWarning}</div> : null}
        ...
        <GraphicEditor postId={post.id} graphicPath={post.graphicPath} topicHeading={post.topicHeading} saved={post.graphicAdjustments} onSaved={(adjustments) => { setPost(current => current ? { ...current, graphicAdjustments: adjustments } : current); setGraphicVersion(version => version + 1); }} />
      </article>
