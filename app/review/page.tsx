...
export default function ReviewPage() {
  ...
  // Unsaved slider positions ride along as a query string; the bump forces a
  // refetch after a save, since the path alone is unchanged.
  const [previewQuery, setPreviewQuery] = useState("");
  const [graphicVersion, setGraphicVersion] = useState(0);
  ...
  const graphicSrc = `${post.graphicPath}${previewQuery ? `?${previewQuery}` : graphicVersion ? `?v=${graphicVersion}` : ""}`;
  ...
  return <>
    ...
    <div className="review-layout">
      <article className="card preview-card">{sourceWarning ? <div className="warning-panel">{sourceWarning}</div> : null}
        ...
        <GraphicEditor postId={post.id} saved={post.graphicAdjustments} onPreviewChange={(query) => { setPreviewQuery(query); setGraphicError(false); }} onSaved={(adjustments) => { setPost(current => current ? { ...current, graphicAdjustments: adjustments } : current); setGraphicVersion(version => version + 1); }} />
      </article>
