...
export function SettingsClient({ rows, links }: { rows: string[][]; links: { label: string; href: string }[] }) {
  ...
  useEffect(() => { fetch("/api/settings").then(r => r.json()).then(data => { setProvider(data.provider); setModel(data.model); setBaseUrl(data.baseUrl || ""); setProviders(data.providers || []); setConfigured(data.configured); setProviderCredentials(data.providerCredentials || {}); setPostiz(data.postiz || { configured: false, apiUrl: "https://api.postiz.com/public/v1" }); setPostizApiUrl(data.postiz?.apiUrl || "https://api.postiz.com/public/v1"); setLoadedApiUrl(data.postiz?.apiUrl || "https://api.postiz.com/public/v1"); }); }, []);
