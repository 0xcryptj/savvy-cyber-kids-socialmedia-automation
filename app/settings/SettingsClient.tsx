...
export function SettingsClient({ rows, links }: { rows: string[][]; links: { label: string; href: string }[] }) {
  ...
  async function save() {
    ...
    const response = await fetch("/api/settings", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider, model, baseUrl, apiKey: apiKey || undefined, postizApiKey: postizApiKey || undefined, postizApiUrl }) });
