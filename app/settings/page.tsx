...
import { getAISettings, providerHasCredential } from "@/src/config/ai-settings";
import { getPostizSettings } from "@/src/config/postiz-settings";

export default async function Settings(){
  const aiSettings = await getAISettings();
  const aiConfigured = await providerHasCredential(aiSettings.provider);
  ...
  const rows = [
    ...
    ["AI captions", aiConfigured ? "Connected" : "Pending", aiConfigured ? `Captions and hashtags use ${aiSettings.provider} · ${aiSettings.model}` : "Set an AI provider key below"],
