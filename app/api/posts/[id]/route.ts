...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  const body = await request.json() as { status?: PostStatus; caption?: string };
  if (body.caption !== undefined && !boundedText(body.caption, 10000)) return NextResponse.json({ error: "Caption is too long" }, { status: 400 });
  try {
    if (body.status) {
      return NextResponse.json(await transitionPost(id, body.status, body.caption ? { caption: body.caption } : undefined));
    }
    const post = await getPost(id);
    if (!post) return NextResponse.json({ error: "Not found" }, { status: 404 });
    return NextResponse.json(await savePost({ ...post, caption: body.caption ?? post.caption }));
  } catch (error) {
